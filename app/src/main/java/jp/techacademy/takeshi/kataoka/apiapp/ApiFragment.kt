...
class ApiFragment: Fragment() {
    ...
    // 「Apiでデータを読込中です」フラグ。追加ページの読み込み時にこれがないと、連続して読み込んでしまうので、それの制御のため
    ...
    override fun onCreateView(
        ...
        return inflater.inflate(R.layout.fragment_api, container, false) // fragment_api.xmlが反映されたViewを作成して、returnする
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        recyclerView.apply {
            ...
            addOnScrollListener(object: RecyclerView.OnScrollListener() { // Scrollを検知するListenerを実装する。これによって、RecyclerViewの下端に近づいた時に次のページを読み込んで、下に付け足す
                override fun onScrolled(recyclerView: RecyclerView, dx: Int, dy: Int) { // dxはx軸方向の変化量（横） dyはy軸方向の変化量（縦） ここではRecyclerViewは縦方向なので、dyだけ考慮する
                    super.onScrolled(recyclerView, dx, dy)
                    if (dy == 0) { // 縦方向の変化量（スクロール量）が0のときは動いていないので何も処理はしない
                        return
                    }
                    ...
                    // totalCountとlastVisibleItemから全体のアイテム数のうちどこまでが見えているかがわかる（例：totalCountが20、lastVisibleItemが15のときは、現在のスクロール位置から下に5件見えていないアイテムがある）
                    // 一番下にスクロールしたときに次の20件を表示する等の実装が可能になる。
                    // ユーザビリティを考えると、一番下にスクロールしてから追加した場合、一度スクロールが止まるので、ユーザーは気付きにくい
                    // ここでは、一番下から5番目を表示した時に追加読み込みするように実装する
    ...
    private fun updateData(isAdd: Boolean = false) {
        ...
        val url = StringBuilder()
            ...
            .append("&format=json") // ここで利用しているAPIの戻りの形をxmlかjsonが選択することができる。Androidで扱う場合はxmlよりもjsonの方が扱いやすいのでjsonを選択
            .toString()
